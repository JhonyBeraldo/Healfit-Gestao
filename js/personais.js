...
function persPeriodoPadrao() {
  ...
  const elIni = document.getElementById('pers-ini');
  const elFim = document.getElementById('pers-fim');
...
async function carregarPersonais() {
  persPeriodoPadrao();
  const pIni = document.getElementById('pers-ini').value;
  const pFim = document.getElementById('pers-fim').value;
  ...
  const [
    ...
    db.rpc('fn_repasses_devidos', { p_ini: pIni, p_fim: pFim }),
  ...
  /* ---------- Cards por personal ---------- */
  const ativos = PERS_LIST.filter(p => p.ativo !== false);
  if (!ativos.length) {
    ...
    grid.innerHTML = ativos.map((p, i) => {
      ...
      <div class="pers-card">
        <div class="pers-acts">
          <button class="icon-btn" title="Editar" onclick="abrirPersonal(${p.id})">✎</button>
          <button class="icon-btn del" title="Excluir" onclick="excluirPersonal(${p.id})">🗑</button>
        </div>
        <div class="pers-top">
        ...
        ${jaPago > 0 ? `<div class="pl" style="margin-top:8px;font-size:12px;color:var(--muted)">Já repassado no período: ${brl(jaPago)}</div>` : ''}
        <div class="pers-foot">
          <button class="btn btn-primary btn-sm" style="flex:1;justify-content:center"
            ...
            onclick="registrarRepasse(${p.id}, ${saldo})">💸 Pagar repasse ${saldo > 0 ? '(' + brl(saldo) + ')' : ''}</button>
        </div>
      </div>`;
    }).join('');
  }
  ...
  const tb = document.getElementById('pers-rep-rows');
  ...
  if (!linhas.length) {
    ...
    tb.innerHTML = linhas.map((m, i) => {
      ...
      return `
      <tr>
        <td><div class="aluno-cell"><div class="av" style="background:${corDe(i)}">${ini(m.personais?.nome)}</div>
  ...
  document.getElementById('pers-rep-total').textContent = 'Liberado no período: ' + brl(totalLiberado);
  ...
  const { data: hist } = await db.from('repasses')
    ...
    .limit(10);
  const tbh = document.getElementById('pers-hist-rows');
  if (!hist || !hist.length) {
...
/* ---------------- REGISTRAR REPASSE ---------------- */
async function registrarRepasse(personalId, valor) {
  const p = PERS_LIST.find(x => x.id === personalId);
  if (!p || valor <= 0) return;
  const pIni = document.getElementById('pers-ini').value;
  const pFim = document.getElementById('pers-fim').value;
...
/* ---------------- NOVO / EDITAR ---------------- */
function abrirPersonal(id) {
  persEditId = id;
  const p = id ? PERS_LIST.find(x => x.id === id) : null;
  document.getElementById('mp-title').textContent = p ? 'Editar personal' : 'Novo personal';
  document.getElementById('mp-nome').value = p?.nome || '';
  document.getElementById('mp-cref').value = p?.cref || '';
  document.getElementById('mp-zap').value = p?.whatsapp || '';
  document.getElementById('mp-pix').value = p?.chave_pix || '';
  document.getElementById('mp-ativo').checked = p ? p.ativo !== false : true;
  openModal('m-pers');
}

async function salvarPersonal() {
  const nome = document.getElementById('mp-nome').value.trim();
  if (!nome) { toast('Informe o nome do personal.'); return; }
  const registro = {
    nome,
    cref: document.getElementById('mp-cref').value.trim() || null,
    whatsapp: document.getElementById('mp-zap').value.trim() || null,
    chave_pix: document.getElementById('mp-pix').value.trim() || null,
    ativo: document.getElementById('mp-ativo').checked,
  };
  let error;
  if (persEditId) ({ error } = await db.from('personais').update(registro).eq('id', persEditId));
  else           ({ error } = await db.from('personais').insert(registro));
  if (error) { toast('Erro ao salvar: ' + error.message); return; }
  closeModal('m-pers');
  toast(persEditId ? 'Personal atualizado ✓' : 'Personal cadastrado ✓');
  carregarPersonais();
}
